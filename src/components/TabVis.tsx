...
export default function StickyHeadTable() {
  ...
  const handleDelete = () => {
    ...
    const confirmation = window.confirm('Êtes-vous sûr de vouloir supprimer ce visiteur ?');
    if (confirmation) {
              ...
              <TableRow>
                {columns.map((column) => (
                  <TableCell
                    key={column.id}
                    align={column.align}
                    style={{ minWidth: isSmallScreen ? 50 : column.minWidth, backgroundColor: '#5D8AA8', color: '#ffffff' }}
                  >
